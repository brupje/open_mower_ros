...
__find_header:
  ...
  if (vecChar.begin() + FRAME_HEAD_SIZE + frame_payload_len + FRAME_CHECKSUM_SIZE + FRAME_END_SIZE + 1 >
      vecChar.end()) {
  ...
  std::vector<uint8_t>(it + FRAME_HEAD_SIZE + frame_payload_len + FRAME_CHECKSUM_SIZE + FRAME_END_SIZE - 1,
                       vecChar.end())
      .swap(vecChar);
  return pf;
